...
MODEL_STATUSES = frozenset({"shadow", "ready", "deprecated", "archived"})
MODEL_TYPES = frozenset({"lora_adapter", "onnx_edge", "federated", "heuristic"})
...
class ModelCard:
    ...
    _ALLOWED_TRANSITIONS: ClassVar[dict[str, frozenset[str]]] = {
        "shadow": frozenset({"ready", "deprecated", "archived"}),
        "ready": frozenset({"deprecated", "archived"}),
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "shadow_until": (
                self.shadow_until.isoformat() if self.shadow_until else None
            ),
